  ...
  async function fetchData() {
    try {
      ...
      const tryFetch = async (orderBy) => {
        if (orderBy) {
          return await supabase.from(table).select('*').order(orderBy, { ascending: false });
        }
      ...
      if (err && (err.code === '42703' || /does not exist/.test(err.message || ''))) {
        console.warn(`${table}: 'created_at' not found, retrying with 'updated_at'`);
        ({ data: result, error: err } = await tryFetch('updated_at'));
      }

      // If still an error, fallback to no ordering
      if (err) {
        console.warn(`${table}: ordering failed, fetching without order`, err.message || err);
